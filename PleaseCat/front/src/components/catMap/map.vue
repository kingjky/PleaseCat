    ...
    mounted(){
        ...
        for (var i = 0; i < this.positions.length; i ++) {
            ...
            if(vm.url === 'catProfile'){
                var marker = new kakao.maps.Marker({
                    map: map, // 마커를 표시할 지도
                    position: new kakao.maps.LatLng(this.positions[i].pos_x, this.positions[i].pos_y), // 마커의 위치
            ...
            kakao.maps.event.addListener(marker, 'click', makeOverListener(this.positions[i].no));
        }
